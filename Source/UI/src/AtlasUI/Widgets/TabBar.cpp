...
void TabBar::addTab(std::string title, std::shared_ptr<IWidget> content) {
...
void TabBar::measure(ILayoutContext& context) {
    for (auto& tab : m_tabs) {
        if (tab.content) tab.content->measure(context);
    }
}

void TabBar::arrange(const NF::Rect& bounds) {
    m_bounds = bounds;
...
void TabBar::paint(IPaintContext& context) {
    if (!m_visible) return;
    ...
    for (size_t i = 0; i < m_tabs.size(); ++i) {
        ...
        context.drawText({rect.x + 8.f, rect.y + 6.f, rect.w - 16.f, rect.h - 12.f},
                         m_tabs[i].title, 0, Theme::ColorToken::Text);
    }
    if (m_activeIndex < m_tabs.size() && m_tabs[m_activeIndex].content &&
        m_tabs[m_activeIndex].content->isVisible()) {
        m_tabs[m_activeIndex].content->paint(context);
    }
}

bool TabBar::handleInput(IInputContext& context) {
...
void TabBar::setActiveIndex(size_t index) {
    if (index < m_tabs.size()) m_activeIndex = index;
}

NF::Rect TabBar::tabRect(size_t index) const {
