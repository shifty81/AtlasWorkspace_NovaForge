...
namespace NF::UI::AtlasUI
{
    ...
    std::optional<CommandId> CommandRegistry::FindByChord(const KeyChord& chord, CommandScope preferredScope) const
    {
        for (const auto& [id, spec] : m_commands)
        {
            if (spec.Scope == preferredScope && (spec.Binding.Primary == chord || spec.Binding.Secondary == chord))
            {
                return id;
            }
        }
