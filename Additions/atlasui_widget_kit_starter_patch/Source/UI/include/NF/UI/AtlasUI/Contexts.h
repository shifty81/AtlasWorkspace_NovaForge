...
public:
    BasicLayoutContext(float dpiScaleValue, Vec2 availableSizeValue)
        : m_dpiScale(dpiScaleValue), m_availableSize(availableSizeValue) {}

    [[nodiscard]] float dpiScale() const override { return m_dpiScale; }
    [[nodiscard]] Vec2 availableSize() const override { return m_availableSize; }
    [[nodiscard]] Vec2 measureText(std::string_view text, float) const override {
...
private:
    ...
    Vec2 m_availableSize{};
    bool m_layoutInvalidated = false;
};

class BasicPaintContext final : public IPaintContext {
public:
    void drawRect(const Rect& rect, Color color) override { m_drawList.push(DrawRectCmd{rect, color}); }
    void fillRect(const Rect& rect, Color color) override { m_drawList.push(FillRectCmd{rect, color}); }
    void drawText(const Rect& rect, std::string_view text, FontId font, Color color) override {
        m_drawList.push(DrawTextCmd{rect, std::string(text), font, color});
    }
    void pushClip(const Rect& rect) override { m_drawList.push(PushClipCmd{rect}); }
...
public:
    ...
    [[nodiscard]] Vec2 mousePosition() const override { return m_mousePosition; }
