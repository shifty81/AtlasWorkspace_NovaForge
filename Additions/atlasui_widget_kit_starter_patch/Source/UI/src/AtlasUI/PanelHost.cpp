...
void PanelHost::detachPanel(std::string_view panelId) {
    auto it = std::remove_if(m_panels.begin(), m_panels.end(), [panelId](const std::shared_ptr<IPanel>& panel) {
...
std::shared_ptr<IPanel> PanelHost::findPanel(std::string_view panelId) const {
    auto it = std::find_if(m_panels.begin(), m_panels.end(), [panelId](const std::shared_ptr<IPanel>& panel) {
        return panel && panelId == panel->panelId();
    });
